...
'''
This program parses jimple files (Soot output) from each provided app
and extracts API methods and exceptions that developers use (to handle called Android API methods).
Then, it gets the API methods and exceptions that are documented in the corresponding 
version of the Android API reference (depending on what API version each app uses).
Finally, it applies set operations on both datasets (app and API)
to find documented/undocumented exceptions.

Input: 
Folder with jimple files (Soot output) from each .apk file (app);
the folder with the jimple files has the same name with the .apk file,
the soot_log.txt file keeps records regarding the processing when Soot is runnig
and it contains the used Android API version.

Output: 
exceptions that are documented/undocumented (android-doc-excps.txt, android-undoc-excps.txt) in the used Android API version.

Instructions to independently run the script:
- before you run the script, cd to ../eRec/apps/apk/ (path for the folder with the output .jimple files)
- in the following program, the path_apk is for the folder with the apps --- .apk files
- in the following program, set ../eRec/api/JSON/ (path for the folder with the JSON files --- different Android API versions)
'''
...
# dictionary of API methods and exceptions in a single .apk
global app_dict
...
p9 = "\$r[0-9]+"
p10 = "[a-z]+[a-zA-Z0-9\$]*\(.*\)"
# pattern to locate API method in jimple files from Android apks 
# (e.g. $r5 = virtualinvoke $r3.<android.view.View: android.view.View findViewById(int)>($i0);)
p11 = "(specialinvoke|staticinvoke|virtualinvoke).*[\<](android)\..*\s[a-z]+[a-zA-Z\$0-9]*\([a-zA-Z\$\_0-9\[\]\.\,\s]*\)"
...
# folder for the jimple files from the .apk files
global path_apk
path_apk = sys.argv[1] # "../eRec/apps/apk/"
# folder for the JSON files (different versions of the Android API)
global path_JSON
path_JSON = sys.argv[2] # "../eRec/api/JSON/"
# Open a new file for doc exceptions
global fi
fi = open("android-doc-excps.txt", "wb")
# Open a new file for undoc exceptions
global fo
fo = open("android-undoc-excps.txt", "wb")

def main():
	# open and parse apk files
	read_folder(path_apk)
	# Close files
	fi.close()
	fo.close()

def read_folder(path):
	# count the number of apps that use valid API version (depending the API versions in JSON)
	count_apps = 0
	app_name = ""
	...
	for k, l in enumerate(dir_list):
		...
		# search for the soot_log.txt file that contains the API version that the app uses
		for f in files:
			c = re.search("soot_log.txt$", f)
			if c:
				# path for the soot_log.txt file
				soot_log = app_path + "/" + c.group()
				api_ver = get_api_ver(soot_log)
				# check if the api version is not available (i.e. < 14)
				if (api_ver == ""):
					# go to the next app
					break
				else:
					...
					doc_dict = get_right_API_version(soot_log)
					read_files(files, app_path, api_ver, doc_dict, app_name)
	print "Number of apps that use valid API versions (v. 14-23): ", count_apps

def read_files(files, app_path, api_ver, doc_dict, app_name):
	for f in files:
		e = (not re.search("^(android)\..*", f)) and re.search("\.jimple$", f)
		if e:
			j_file = app_path + "/" + f
			parse_jimple(j_file, doc_dict, app_name, api_ver)
		# these files are the Android files found in the .apk
		d = re.search("^(android)\..*", f) and re.search("\.jimple$", f)
		if d:
			l = app_path + "/" + f

# get the version of the API platform that the examined app uses
# (the version of the used Android platform can be found in the soot_log.txt file, 
# which is in the folder with the jimple files of each .apk file)
def get_right_API_version(file):
	android_ver = ""
	api_dict = OrderedDict([])
	# read the soot_log.txt file and get the API version
	api_ver = get_api_ver(file)
	api_dict = read_right_javadoc_version(api_ver)
	return api_dict

def get_api_ver(file):
	api_ver = ""
	# read the soot_log.txt file and get the API version
	f = open(file)
	lines = f.readlines()
	for k, l in enumerate(lines):
		# search for the pattern: Using './android-17/android.jar' as android.jar
		if (re.search("^Using.*(android-)(1[4-9]|2[0-3])+.*", lines[k])):
			api_ver1 = re.split("\s+", lines[k])
			api_ver2 = re.split("\/", api_ver1[1])
			api_ver = api_ver2[len(api_ver2) - 2]
	return api_ver

# find and read the right JSON file for the corresponding API version
def read_right_javadoc_version(android_ver):
	if (android_ver) is not None :
		is_api_version = False
		json_file = android_ver + ".json"
		...
		if (is_api_version == False):
			print 'Not found API version: ', android_ver
			raise IOError
...
def update_md_args(method_sig):
	...
	# split the method args to seek android case
	l_args = re.split(",", m_args[1])
	for l, k in enumerate(l_args):
		n_sp = re.sub(" ", "", l_args[l])
		if (re.search("android", n_sp) and not re.search("\$", n_sp)):
		...
		elif (re.search("android", n_sp) and re.search("\$", n_sp)):
			n_eb = re.sub("\$", ".", n_sp)
...
# find method calls to the Android API in each app method's body
def isolate_api_method(line):
	new_api_method = ""
	line_spl = re.split("\<", line)
	android_elems = re.split("\:", line_spl[1])
	android_cls = android_elems[0]
	if re.search(p1, android_elems[1]):
		method_elems = re.split("\>", re.search(p1, android_elems[1]).group())
		android_method = re.sub(" ", "", method_elems[0])
		android_mthd_cls = android_cls + "." + android_method
		new_api_method = update_md_args(android_mthd_cls)
